...
(function ($) {
  $.fn.slideshow = function (options) {
    ...
    return this.each(function () {
      ...
      var index = 1;
      var timer = false;
      var run = $.fn.slideshow[settings.effect]; //Initialize
      ...
      function mouseHandler() {
        var isRunning;
        $slides.add($tabs).add($prevButton).add($nextButton).mouseover(function () {
          isRunning = timer;
          stopTimer();
        }).mouseout(function () {
  ...
  $.fn.slideshow.slide = function ($this, settings, direction, index, jumpTo) {
    ...
    var speed = settings.speed; //Clicked on tab
    ...
    if (jumpTo != undefined) {
      ...
      if ($this.css("left") <= width * -numSlides + "px") {
        //append start to end
        $this.children(":first").clone().appendTo($this); //animate slide
